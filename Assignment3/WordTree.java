...
public class WordTree
{
	...
	public void insert(String word)
	{
		...
		WordTreeNode longestPrefix = getPrefixNode(word);

		//Level corresponds to where the tracking starts, which is the depth of the longestPrefix
		for(int level = longestPrefix.depth; level < word.length(); level++){

			//Create a child node extending from the end of the longest prefix
			longestPrefix = longestPrefix.createChild(word.charAt(level));
		}
		//Set that child to be the end of the tree
		longestPrefix.setEndOfWord(true);
	...
	WordTreeNode getPrefixNode(String word)
	{
		...
		//Use while loop for the length of the word

		//Create a tracker for the current node you're at, starting at root
		//Root is already set as ""
		WordTreeNode current = root;
		int indexPosition = 0;

		while(indexPosition < word.length()){

			//Checks to see if the next letter has children (important for finding the prefix)
			//If null then prefix is found, if empty then word is not in list
			if(current.getChild(word.charAt(indexPosition)) != null){
				current = current.getChild(word.charAt(indexPosition));
			}
			else{
				return current;
			}
	...
	{  
		...
		//Code only true for full words within the tree
		WordTreeNode currentNode = getPrefixNode(word);

		if(currentNode != null && currentNode.isEndOfWord() && currentNode.depth == word.length()){
			return true;
		}
		return false;
	...
	{
		//  ADD YOUR CODE BELOW 
		//Has to be recursive (Depth first search)


		ArrayList<String> stringList = new ArrayList<>();

		//Have to get all words that have this prefix so we have to use previous method and store the prefix
		WordTreeNode node = getPrefixNode(prefix);

		if(node.depth == prefix.length()){
			depthFirst(stringList, node);
		}
	...
	public class WordTreeNode
	{
		...
		{	   
			...
			//Child node must have a reference back to itself
			child.parent = this;

			//Add child to the array based on ascii value
			children[c] = child;

			//Increase depth of child character & connection between node and parent node
			child.depth = depth + 1;
	...
	//Used within wordtree class
	public void depthFirst(ArrayList<String> list, WordTreeNode currentNode){

		//Every time it's called, check to see if you are at the bottom of the tree
		//And add the word to the list
		if(currentNode.isEndOfWord()){
			list.add(currentNode.toString());
		}
		//Iterate through the ascii values in order to cover all the words inserted inside the tree
		for(int i = 0; i < WordTreeNode.NUMCHILDREN; i++){

			//If there is a child to the current node, keep going until you reach the end key

			if(currentNode.getChild((char)i) != null){

				depthFirst(list, currentNode.getChild((char) i));
			}
		}
	}
